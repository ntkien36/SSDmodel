...
def show_predict(img_file_path):
    ...
    img_tensor = torch.from_numpy(img_tranformed[:,:,(2,1,0)]).permute(2,0,1) #đưa channel lên đầu, tso chiều cao, rộng đế sau

    net.eval()
    input = img_tensor.unsqueeze(0) #(1, 3, 300, 300): 1 ảnh, 3 channel, 300x300
    output = net(input)
    ...
    detections = output.data #(1, 21, 200, 5) 1 ảnh, 21 classes, 200 bounding box, 5: score, cx, cy, w, h
